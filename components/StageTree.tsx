...
export default function StageTree({ stage, current = false }: StageTreeProps) {
      ...
      case 5:
        return (
          <g opacity={opacity}>
            ...
            <path d={`M ${5*scale} ${65*scale} Q ${2*scale} ${75*scale} ${5*scale} ${85*scale}`} stroke={BRANCH_DIM} strokeWidth={1.2*scale} strokeLinecap="round" fill="none" opacity="0.5" />
            <path d={`M ${75*scale} ${65*scale} Q ${78*scale} ${75*scale} ${75*scale} ${85*scale}`} stroke={BRANCH_DIM} strokeWidth={1.2*scale} strokeLinecap="round" fill="none" opacity="0.5" />
            <path d={`M ${8*scale} ${48*scale} Q ${5*scale} ${55*scale} ${8*scale} ${62*scale}`} stroke={BRANCH_DIM} strokeWidth={1*scale} strokeLinecap="round" fill="none" opacity="0.4" />
            <path d={`M ${72*scale} ${48*scale} Q ${75*scale} ${55*scale} ${72*scale} ${62*scale}`} stroke={BRANCH_DIM} strokeWidth={1*scale} strokeLinecap="round" fill="none" opacity="0.4" />
            {current && <circle cx={40*scale} cy={25*scale} r={9*scale} fill={GOLD} opacity="0.12" />}
            <motion.circle
              cx={40*scale}
              cy={25*scale}
              r={5*scale}
            ...
            <circle cx={5*scale} cy={85*scale} r={2*scale} fill={SAGE} opacity="0.5" />
            <circle cx={75*scale} cy={85*scale} r={2*scale} fill={SAGE} opacity="0.5" />
          </g>
        );
      default:
        return null;
  ...
  return (
    <svg viewBox="0 0 80 100" className="w-full h-full overflow-visible">
      <ellipse cx={40*scale} cy={95} rx={10 * (current ? 1 : 0.7)} ry={2 * (current ? 1 : 0.7)} fill={SAGE} opacity={0.08 * opacity} />
      {renderStage()}
    </svg>
  );
}
